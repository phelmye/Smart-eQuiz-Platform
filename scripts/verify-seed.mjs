...
const expectMaps = process.env.EXPECT_MAPPINGS ? parseInt(process.env.EXPECT_MAPPINGS, 10) : 1;
...
async function main() {
  try {
    ...
    if (tours < expectTours) throw new Error(`tournaments count ${tours} < expected ${expectTours}`);
    if (ques < expectQues) throw new Error(`questions count ${ques} < expected ${expectQues}`);
    if (maps < expectMaps) throw new Error(`mappings count ${maps} < expected ${expectMaps}`);
